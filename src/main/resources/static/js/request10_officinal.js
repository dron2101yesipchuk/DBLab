...
app.controller("AppCtrl", function ($http, $scope){
    $scope.request10 = [];
    var med_id = 1;
    $http.get('http://localhost:8080/api/request10_officinal?id='+med_id)
        .then(function (response){
            $scope.request10=response.data;
            console.log(response);
            $http.get('/api/for_request10_officinal').then(function (response){
    ...
    this.update_request = function add() {
        ...
        $http.get('http://localhost:8080/api/request10_officinal?id='+med_id).then(function (response){
            $scope.request10=response.data;
            console.log(response);
        });
    };
